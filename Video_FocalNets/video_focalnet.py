...
def videofocalnet_base(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load('Video_FocalNets\\video-focalnet_base_kinetics400.pth')['model'])
    return model

videofocalnet_b = videofocalnet_base(pretrained=True, num_classes=400)
videofocalnet_b.head = nn.Identity()
print("TEST OUTPUT: ", videofocalnet_b(torch.zeros(4, 8, 3, 224, 224)).shape)
print("param", sum(p.numel() for p in videofocalnet_b.parameters() if p.requires_grad))